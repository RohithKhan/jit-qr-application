        ...
        try {
            ...
            if (response.status === 200) {
                await AsyncStorage.multiSet([
                    ...
                    ['isLoggedIn', 'true'],
                    ['userType', 'year_incharge'],
                ]);
                Toast.show({ type: 'success', text1: 'Login successful!' });
                setTimeout(() => navigation.reset({ index: 0, routes: [{ name: 'YearIncharge' }] }), 800);
            }
        } catch (error: any) {
        ...
        <SafeAreaView style={styles.container}>
            <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.card}>
                    ...
                    <Text style={styles.title}>Year Incharge Login</Text>
                    <Text style={styles.subtitle}>Enter your year incharge credentials</Text>
                    <View style={styles.inputGroup}>
                        <Text style={styles.label}>Email</Text>
                        <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder="Email" placeholderTextColor={COLORS.textLight} keyboardType="email-address" autoCapitalize="none" />
                    </View>
                    <View style={styles.inputGroup}>
...
const styles = StyleSheet.create({
    ...
    title: { fontSize: 24, fontWeight: '800', color: '#7c3aed', textAlign: 'center', marginBottom: 6 },
    ...
    eyeBtn: { padding: 12 },
    btn: { backgroundColor: '#7c3aed', paddingVertical: 16, borderRadius: 16, alignItems: 'center', marginTop: 12, ...SHADOWS.medium },
    btnDisabled: { opacity: 0.7 },
    btnText: { color: COLORS.white, fontSize: 16, fontWeight: '700' },
});

export default YearInchargeLoginScreen;
